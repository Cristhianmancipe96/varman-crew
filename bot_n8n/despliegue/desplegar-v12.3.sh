...
NUEVO=/home/andre/bot-varman-v12.3.json
TAM_ESPERADO=635039
PALABRA=BOT_LINK_320          # solo existe en la v12.3
ID=VarmanBotV4Ped01
HOY=2026-09-07
...
BAK=/home/andre/bot-varman-ANTES-DE-v12.3-$HOY.json
if [ -f "$BAK" ]; then
  echo "el respaldo ya existe, no se toca: $BAK"
else
  sudo sh -c "cd $D && docker compose run --rm --no-deps -u node n8n export:workflow --id=$ID --output=/workflows/respaldo/ANTES-DE-v12.3-$HOY.json" 2>&1 | tail -3
  sudo cp "$D/workflows/respaldo/ANTES-DE-v12.3-$HOY.json" "$BAK" 2>/dev/null
fi
ls -l "$BAK" 2>/dev/null || echo "  (no se pudo exportar de la base; queda el respaldo del archivo)"
sudo cp "$D/workflows/bot-varman.json" "/home/andre/bot-varman-ARCHIVO-ANTES-DE-v12.3-$HOY.json"
ls -l "/home/andre/bot-varman-ARCHIVO-ANTES-DE-v12.3-$HOY.json"
